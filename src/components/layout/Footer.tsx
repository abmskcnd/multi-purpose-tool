    ...
    <footer className="border-t bg-muted/30">
      <div className="container mx-auto px-4 py-12">
        <div className="grid gap-8 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <div>
            <h3 className="mb-4 text-sm font-semibold">{footerT('stay_updated')}</h3>
            <p className="mb-4 text-sm text-muted-foreground">
              {footerT('newsletter_description')}
            </p>
            <form className="flex gap-2" onSubmit={(e) => e.preventDefault()}>
              <input
                type="email"
                placeholder={footerT('email_placeholder')}
        ...
        <div className="mt-12 flex flex-col items-center justify-between gap-4 border-t pt-8 sm:flex-row">
          <p className="text-sm text-muted-foreground">
            © {currentYear} {t('site_name')}. {t('all_rights_reserved')}
          </p>
          <p className="text-sm text-muted-foreground">
            {footerT('made_with')} <span className="text-red-500">❤</span> {footerT('by_team')}
          </p>
        </div>
      </div>
    </footer>
